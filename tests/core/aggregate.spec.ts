...
describe("aggregate", () => {
  describe("aggregate native methods", () => {
    ...
    it("should create a valid aggregate", () => {
      const obj = AggregateErr.create({
        id: "23366cbf-86cd-4de3-874a-5a11b4fe5dac",
        name: "Jane",
      });
      expect(obj.isFail()).toBeFalsy();
      expect(obj.value().get("name")).toBe("Jane");
      expect(obj.value().hashCode().value()).toBe(
        "[Aggregate@AggregateErr]:23366cbf-86cd-4de3-874a-5a11b4fe5dac",
      );
    });
  });
  ...
  describe("basic-aggregate", () => {
    ...
    it("should create a basic aggregate with a provided id", () => {
      ...
      expect(agg.value().hashCode().value()).toBe(
        "[Aggregate@BasicAggregate]:8b51a5a2-d47a-4431-884a-4c7d77e1a201",
      );
    });
  ...
  describe("aggregate with value objects", () => {
    ...
    class UserAgg extends Aggregate<AggProps> {
      ...
      public static create(
        props: AggProps,
      ): Result<Aggregate<AggProps> | null> {
    ...
    it("should get value from age with success", () => {
      ...
      const result = (user as Aggregate<AggProps>).get("age").get("value");
  ...
  describe("createdAt and updatedAt", () => {
    ...
    it("should create a date from props if provide value", () => {
      ...
      expect(agg.value().get("createdAt")).toEqual(
        new Date("2022-01-01T03:00:00.000Z"),
      );
      expect(agg.value().get("updatedAt")).toEqual(
        new Date("2022-01-01T03:00:00.000Z"),
      );
    });
    ...
    it("should update a the value of updatedAt if change some prop", () => {
      ...
      expect(agg.value().get("updatedAt")).not.toEqual(
        new Date("2022-01-01T03:00:00.000Z"),
      );
    });
  ...
  describe("aggregate with domain id", () => {
    it("should be success if define id as UID", () => {
      ...
      expect(result.value().id.value()).toBe(
        "fd15df0c-af60-45ce-9976-33c6197e5ca0",
      );
      expect(result.value().get("id").value()).toBe(
        "fd15df0c-af60-45ce-9976-33c6197e5ca0",
      );
    });
  });
...
describe("Aggregate", () => {
  ...
  describe("context", () => {
    it("should return an EventManager", () => {
      ...
      expect(context.removeEvent).toBeDefined();
  ...
  describe("clearEvents", () => {
    it("should delete all events in the current aggregate instance", () => {
      ...
      const clearEventsSpy = jest
        .spyOn(events, "clearEvents")
        .mockImplementation(() => {});
