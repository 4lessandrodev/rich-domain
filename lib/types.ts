...
export interface _Entity<Props> {
  ...
  toObject<T>(
    adapter?: _Adapter<_Entity<Props>, any>,
  ): T extends {}
...
export interface _EntityGettersAndSetters<Props> {
   ...
   */
  change<Key extends keyof Props>(
    key: Key,
    value: Props[Key],
    validation?: (value: Props[Key]) => boolean,
  ): boolean;
   ...
   */
  validation<Key extends keyof Props>(
    value: Props[Key],
    key: Props extends object ? Key : never,
  ): boolean;
}
...
type SerializerEntityReturnType<ThisEntity extends Entity<any>> = ReturnType<
  ThisEntity["getRaw"]
>;
type SerializerValueObjectReturnType<ThisValueObject extends ValueObject<any>> =
  ReturnType<ThisValueObject["getRaw"]>;
      ...
      : Props[key] extends Array<any>
        ? Array<
            AutoMapperSerializer<ReturnType<Props[key][0]["getRaw"]>> &
              (Props[key][0] extends Entity<any> ? EntityMapperPayload : {})
          >
        : Props[key];
};
...
export interface _AutoMapper<Props> {
  ...
  valueObjectToObj(
    valueObject: _ValueObject<Props>,
  ): AutoMapperSerializer<Props>;

  /** Maps an entity to a serializable object with metadata. */
  entityToObj(
    entity: _Entity<Props>,
  ): AutoMapperSerializer<Props> & EntityMapperPayload;
}
